	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			"mcp.mode.dynamic.desc": "允许的服务器默认不挂载；需要时用 mcp_load 把它挂进当前会话。工具列表会变化一次，请求缓存前缀随之中断。配了工具过滤的行只挂可见的工具，参数 schema 照旧。",
			"mcp.mode.lazy": "延迟加载",
		...
		const en = {
			...
			"mcp.mode.dynamic.desc": "Allowed servers stay unmounted by default; mcp_load mounts one into the calling session. The tool list changes once per load, which breaks the request-cache prefix. A filtered row mounts only its visible tools, with their real schemas.",
			"mcp.mode.lazy": "Lazy",
