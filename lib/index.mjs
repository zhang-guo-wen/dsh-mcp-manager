...
function readMcpRoster(ctx, mountReader) {
	...
	if (loader !== void 0) for (const entry of loader.entries()) {
		if (entry.options.name === "cordis:include" && entry.subtree !== void 0) includes.push({ patches: entry.subtree.config?.patches?.length ?? 0 });
	...
	const mounts = mountReader(ctx.root.fiber);
	const presets = listPresetDeclarations(ctx).map((declaration) => {
	...
	const rootInclude = includes.length === 1 ? includes[0] : void 0;
	const problem = rootInclude === void 0 ? "no-include" : rootInclude.patches > 0 ? "patched-include" : void 0;
	return {
		entries,
		presets,
		globalWritable: problem === void 0,
		...problem === void 0 ? {} : { globalProblem: problem }
	};
}
//#endregion
